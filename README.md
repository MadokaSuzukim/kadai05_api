...
-一斉にログインした際の検証をしたい
-ログイン後にタグ付けしたスタッフのみ、全員の位置情報が記載された地図をみることができる。
-利用者さんの画面で、ピンをユーザーが選んだものを表示：自宅にいます。避難所にいます。家族が離れていますなど状況に応じてピンを色分けしたい。
-スタッフサイドでは、確認済みなのか未確認なのかわかるようにする。未確認な場合誰が追っていくのかタグ付けさせる
...
6. テストと訓練
   ...
   このようなシステムを構築するには、技術的な実装だけでなく、法律やプライバシーの問題、行政や救助機関との連携など、多岐にわたる検討が必要。
